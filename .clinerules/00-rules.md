# .clinerules/00-rules.md — Aturan Main untuk Cline

> File ini adalah salinan `AGENTS.md` dengan tambahan catatan khusus Cline
> di bagian atas. Kalau `AGENTS.md` berubah, file ini **wajib** ikut disalin
> ulang: `cp AGENTS.md .clinerules/00-rules.md`, lalu tempel kembali bagian
> "Catatan Khusus Cline" di bawah ini.

---

## Catatan Khusus Cline

Kamu memegang `client/**` saja. Kamu **tidak pernah** menyentuh `server/**`,
`ai-service/**`, atau `.github/**` — kalau UI butuh field baru dari API,
buka issue berlabel `agent:claude-code`, jangan menebak bentuk response.

**Sumber kebenaran kontrak API kamu:** `ARCHITECTURE.md §3` (format response
standar) dan `§4` (daftar endpoint). Jangan mengarang field yang tidak ada
di sana, walaupun kelihatan masuk akal.

**Stack yang dikunci** (`SPEC.md §10`): React 18 + Vite, React Router, Axios,
Socket.IO client, CSS biasa/CSS Modules + Tailwind, `recharts` (hanya untuk
grafik admin). **Dilarang** menambah Redux, Zustand, React Query, atau UI
library (MUI, Chakra, dll) di v1 — state cukup React state + Context.

**Test kamu** (`TESTING.md §10`, kamu pemiliknya): Vitest + React Testing
Library. Coverage minimum 75% untuk `src/components` & `src/pages`, 90%
untuk `src/services/api.js`. Setiap halaman wajib punya test 4 keadaan:
loading, kosong, error, sukses. Wajib ada test untuk **409 saat booking →
daftar slot dimuat ulang otomatis** (bukan pesan error merah lalu diam), dan
test bahwa **setiap tampilan ringkasan AI memuat disclaimer**, dan bahwa
signed URL dokumen **tidak** disimpan di `localStorage` atau state global
yang bertahan lintas halaman — ini rawan bocor.

**Yang paling sering kamu salah di project sejenis ini:**
1. Menyimpan signed URL dokumen di state yang bertahan lama / localStorage
2. Menampilkan ringkasan AI tanpa disclaimer saat status bukan `done`
   (harusnya tampilkan "ringkasan tidak tersedia" untuk `rejected`/`failed`)
3. Menangani 409 booking dengan alert generik, bukan reload daftar slot
4. Menulis `if (user.role === ...)` di komponen untuk logika akses — itu
   keputusan backend (`accessService.js`), frontend hanya menampilkan apa
   yang dikirim API
