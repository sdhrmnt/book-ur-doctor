# CLAUDE.md — Aturan Main untuk Claude Code

> File ini adalah salinan `AGENTS.md` dengan tambahan catatan khusus Claude Code
> di bagian atas. Kalau `AGENTS.md` berubah, file ini **wajib** ikut disalin ulang:
> `cp AGENTS.md CLAUDE.md`, lalu tempel kembali bagian "Catatan Khusus" di bawah.

---

## Catatan Khusus Claude Code

Kamu memegang `server/**` dan `ai-service/**`. Ini artinya semua invariant paling
mahal di project ini ada di tanganmu.

**Empat file yang paling menentukan, tulis dan review sepelan mungkin:**

| File | Kenapa |
|---|---|
| `server/services/availabilityService.js` | Salah sedikit → pasien datang ke jadwal yang tidak ada |
| `server/services/accessService.js` | Salah sedikit → rekam medis orang lain terbuka |
| `server/services/documentService.js` | Signed URL + audit log, keduanya tidak boleh terlewat |
| `ai-service/deidentify.py` | Satu-satunya alasan kita boleh mengirim dokumen ke provider LLM |

**Sebelum menulis service baru:** baca `server/services/accessService.js` dan
`server/__tests__/unit/accessService.test.js`, lalu tiru strukturnya persis.
Coverage kedua file itu dipatok 100%.

**Sebelum menyentuh `ai-service`:** baca `ARCHITECTURE.md §6`. Urutan pipeline
`extractor → deidentify → hash → summarizer → guardrail` tidak boleh diubah,
dilewati, atau dibalik. Kalau kamu menambah endpoint baru di FastAPI, endpoint itu
wajib melewati `deidentify.py` juga.

**Yang paling sering kamu salah di project ini** (urut dari yang paling sering):
1. Membuat tabel atau cache `Slots`
2. Mencegah double-booking dengan `findOne` lalu `create`
3. `if (user.role === ...)` bocor keluar dari `accessService`
4. `new Date()` di dalam service
5. Menerbitkan signed URL tanpa menulis `DocumentAccessLogs`

Kalau kamu menemukan dirimu menulis salah satu dari lima ini, berhenti dan baca
ulang `SPEC.md §4`, `SPEC.md §6`, dan `ERD.md §3`.

**Batas wilayah:** kamu tidak menyentuh `client/**` dan `.github/**`. Kalau
frontend perlu field baru di response, buka issue untuk Cline — jangan mengubah
`ARCHITECTURE.md` sendiri, itu milik manusia.
